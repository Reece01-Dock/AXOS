...
# On a live squashfs root we cannot rewrite /usr/sbin — JFFS services-start
# remains the primary hook until the next firmware image that includes this
# binary under /usr/sbin/axos-bootstrap.
set -eu
...
if ! AXOS_ROOT="$(resolve_axos_dir)"; then
  # Nothing installed yet — silent success so boot hooks don't fail.
  exit 0
fi

AXOSD="$AXOS_ROOT/bin/axosd"
# Prefer the install root (/jffs/axos) for mutable state even when the
# binary lives under current/ (deploy symlink layout).
...
mkdir -p "$STATE_DIR/backups" "$STATE_DIR/logs" "$STATE_DIR/www"
...
# Bound to loopback only — see docs/security.md.
